...
class M_produksi extends CI_Model
{
	...
	public function laporan_produksi($mulai, $hingga)
	{
		...
		from "' . $this->database_name . '"."IGN1" A
		Left Join(select "DocNum","DocEntry","DocDate","Comments" from "' . $this->database_name . '"."OIGN")B on B."DocEntry" = A."DocEntry"
	...
	{
		$hanadb = $this->load->database('hana', TRUE);
		return $hanadb->query('select A."ItemCode" ||' . "'|'" . '|| A."Dscription" ||' . "'|0|1'" . ' as "ItemCode" from "' . $this->database_name . '"."IGN1" A
		Left Join(select "DocNum","DocEntry","DocDate","Comments","CANCELED" from "' . $this->database_name . '"."OIGN")B on B."DocEntry" = A."DocEntry"
	...
	public function get_item_master()
	{
		...
		Left join (select "UgpEntry","UgpCode" from "' . $this->database_name . '"."OUGP") B on B."UgpEntry" = A."UgpEntry"
		Left join (select "ItmsGrpCod","ItmsGrpNam" from "' . $this->database_name . '"."OITB") C on C."ItmsGrpCod" = A."ItmsGrpCod"
