...
class M_so extends CI_Model
{
    ...
    public function so_head($mulai,$hingga)
    {
        ...
        "t1"."Price" AS "DPP",
        ("t1"."Quantity"- COALESCE((SELECT SUM ("Quantity") FROM "BKI_LIVE".DLN1 WHERE "BaseEntry"  = "t1"."DocEntry" AND "BaseLine"  = "t1"."LineNum" AND "BaseType"  = "t1"."ObjType" ),0)) AS "Sisa_SO",
      ...
      FROM "BKI_LIVE"."ORDR" AS "t0" 
      Left JOIN "BKI_LIVE"."RDR1" AS "t1" ON "t0"."DocEntry"  = "t1"."DocEntry"
    ...
    FROM "BKI_LIVE"."ORDR" AS "t0" 
    Left JOIN "BKI_LIVE"."RDR1" AS "t1" ON "t0"."DocEntry"  = "t1"."DocEntry"
    Left JOIN "BKI_LIVE"."DLN1" AS "t2" ON "t2"."BaseEntry"  = "t1"."DocEntry" AND "t2"."BaseLine"  = "t1"."LineNum" AND "t2"."BaseType"  = "t1"."ObjType"
    Left JOIN(SELECT "DocEntry","DocDate","DocNum","CANCELED" FROM "BKI_LIVE"."ODLN")"t3" on "t2"."DocEntry" = "t3"."DocEntry"
    ...
    public function trace_sj_head($id)
    {
        ...
        "CardCode","CardName","NumAtCard" from "BKI_LIVE"."ODLN" where "DocNum" = '.$id.';')->result_array();
    }
    ...
    public function trace_sj_detail($id)
    {
        ...
        from "BKI_LIVE"."ODLN" A
        Left Join "BKI_LIVE"."DLN1" B on B."DocEntry" = A."DocEntry"
    ...
    public function lhmb($mulai,$hingga)
    {
		...
		TO_VARCHAR (right(year(TO_DATE(A."DocDate")),2)) as "DocDate",A."CardCode",A."CardName" as "Customer",
        B."DocEntry",B."ItemCode",B."Dscription",B."Quantity",B."UomCode",B."FreeTxt"
        from "BKI_LIVE"."OWTR" A 
        Left Join "BKI_LIVE"."WTR1" B on B."DocEntry" = A."DocEntry"
    ...
    public function hitung_so($mulai,$hingga,$cari)
	{
		...
		from "BKI_LIVE"."ORDR" where "DocDate" between '."'$mulai'".' and '."'$hingga'".' and "CardName" like '."'%$cari%'".';')->row_array();
		return $query['row'];
	}
    ...
    public function get_so($no_page,$mulai,$hingga,$cari)
	{
		...
		"DocCur",
		"DocTotal"
		from "BKI_LIVE"."ORDR" where "DocDate" between '."'$mulai'".' and '."'$hingga'".' and "CardName" like '."'%$cari%'".'
    ...
    public function detail_so($no_page,$mulai,$hingga,$cari)
	{
		...
		Left join(select "DocEntry","ItemCode","Dscription","Quantity","UomCode","Currency","Price","GTotal" from "BKI_LIVE"."RDR1")B on B."DocEntry" = "tb_so"."DocEntry"
