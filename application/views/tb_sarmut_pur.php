...
$pr_data = json_decode($pr_data, true);
?>
<hr>
<table class="table table-sm table-bordered" width="100%" border="1" rules="all" style="font-size:13px;">
    ...
    <tbody>
        ...
        foreach ($row['detail'] as $h) {
            ...
            // Memeriksa apakah id_sarmut saat ini ada dalam array $pr_data
            if (isset($pr_data[$h['id_sarmut']])) {
                // Jika ada, maka tampilkan input text dengan nilai dari $pr_data
                echo "<td><input type='text' class='form-control' id='nilai' name='nilai[]' value='" . intval($pr_data[$h['id_sarmut']]['Persentase_' . $h['id_sarmut']]) . "%' readonly></td>";
            } else {
                // Jika tidak, tampilkan input text biasa
                echo "<td><input type='text' class='form-control' id='nilai' name='nilai[]'></td>";
            }
