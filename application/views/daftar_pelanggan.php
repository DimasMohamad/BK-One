                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-xl-12">
                                <br>
                                <div id="tampildatapelanggan"></div>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    function tampildata(){
        ...
        $.get("<?= base_url('Marketing/daftar_pelanggan?mulai=') ?>"+mulai+"&hingga="+hingga, function(data, status) {
            document.getElementById('btntampil').style.display = '';
            document.getElementById('btnloading').style.display = 'none';
            $("#tampildatapelanggan").html(data);
        });
    }
    ...
    function printData() {
        let mulai = $("#mulai").val();
        let hingga = $("#hingga").val();
        window.open("<?= base_url('Marketing/print_pelanggan?mulai=') ?>"+mulai+"&hingga="+hingga,"_blank");
    }
    
    function printData2(CardCode) {
        //swaL.alert(CardCode);
        window.open("<?= base_url('Marketing/print_pelanggan?CardCode=')?>"+CardCode);
    }
</script>