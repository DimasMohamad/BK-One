    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Outstanding SO</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
...
<!-- Trace SJ -->
<div class="modal fade" id="trace_sj" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
...
<script>
    function tampildata(){
        ...
        $.get("<?= base_url('So/tb_outs_so?mulai=') ?>"+mulai+"&hingga="+hingga, function(data, status) {
    ...
    function unduh(){
        let mulai = $("#mulai").val();
        let hingga = $("#hingga").val();
        window.open("<?= base_url('so/tb_outs_so_xls?mulai=') ?>"+mulai+"&hingga="+hingga,"_self");
    }

    function trace_so(id){
        document.getElementById('loading2').style.display = '';
        document.getElementById("tampilso").innerHTML = '';
        $("#trace_sj").modal("show");
        
        $.get("<?= base_url('So/trace_sj?docnum=') ?>"+id, function(data, status) {
            ...
            if(status == 'success'){
                var table = '<table class="table table-sm table-bordered" style="font-size:13px;">'+
                    ...
                    $.each(obj['head'], function(key, row) {
                        ...
                        table += '</tr>';
                        table += '<tr>';
                        table += '<th>No Delivery</th>';
                        table += '<th colspan="5">'+row['DocNum']+'</th>';
                        table += '</tr>';
                        table += '<tr>';
                        table += '<th>Posting Date</th>';
                        table += '<th colspan="5">'+row['DocDate']+'</th>';
                        table += '</tr>';
                        table += '<tr>';
                        table += '<th>No PO</th>';
                        table += '<th colspan="5">'+row['NumAtCard']+'</th>';
                        table += '</tr>';
                    });
                    table += '<tr>'+
                        '<th>Item Code</th>'+
                        ...
                        '<th>Free Text</th>'+
                        '</tr></thead><tbody>';
                    $.each(obj['item'], function(key, row) {
                        table += '<tr>';
                    ...
                    document.getElementById("tampilso").innerHTML = table;   
            }
        });
