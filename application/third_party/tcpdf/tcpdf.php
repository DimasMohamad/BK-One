...
class TCPDF
{
	...
	public function Header()
	{
		if ($this->header_xobjid === false) {
			...
			// Konversi nilai ke tipe data float
			$logo_width = floatval($headerdata['logo_width']);

			// Get the right-to-left (RTL) setting
			$rtl = $this->getRTL();

			// Calculate header_x based on RTL setting
			if ($rtl) {
				// If RTL is enabled, adjust header_x based on the right margin and logo width
				$header_x = $this->original_rMargin + ($logo_width * 1.1);
			} else {
				// If RTL is not enabled, adjust header_x based on the left margin and logo width
